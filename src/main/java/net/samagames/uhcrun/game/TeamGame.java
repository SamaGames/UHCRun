...
public class TeamGame extends BasicGame {
    ...
    public void startGame() {
        ...
        while (iterator.hasNext()) {
            ...
            if (getPlayerTeam(id) == null) {
                ...
                if (getPlayerTeam(id) == null) {
                    api.getGameManager().kickPlayer(player.getPlayer(), ChatColor.RED + "Aucune team était apte à vous reçevoir, vous avez été réenvoyé dans le lobby.");
                }
            }
        }
    ...
    public void creditKillCoins(Player player) {
        ...
        Team team = getPlayerTeam(player.getUniqueId());
        if (team != null) {
            team.getPlayers().stream().filter(id -> !id.equals(player.getUniqueId())).forEach(id -> SamaGamesAPI.get().getPlayerManager().getPlayerData(player.getUniqueId()).creditCoins(10, "Votre équipe fait un kill !", true));
        }
    }
    ...
    public void win(final Team team) {
        ...
        for (final UUID playerID : team.getPlayers()) {
            SamaGamesAPI.get().getPlayerManager().getPlayerData(playerID).creditCoins(100, "Victoire !", true);
            SamaGamesAPI.get().getPlayerManager().getPlayerData(playerID).creditStars(2, "Victoire !", true);

            try {
                api.getStatsManager("uhcrun").increase(playerID, "victories", 1);
            } catch (Exception ignored) {}
